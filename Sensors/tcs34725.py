...
from machine import Pin, I2C  # I/O
...
class TCS34725(object):
    ...
    def __init__(self, addr=TCS34725_I2C_ADDR, scl=None, sda=None, freq=TCS34725_FREQ):
        ...
        if not (isinstance(scl, Pin) and isinstance(sda, Pin)):
            ...
            return  # must be specified as Pin objects
        self.__bus = I2C(scl=scl, sda=sda, freq=freq)  # software driver
        try:
    ...
    @property
    def rgb(self):
        """ read all data registers, return tuple: (red, green, blue) """
        clear, red, green, blue = self.colors
        red = int(pow((int((red / clear) * 256) / 255), 2.5) * 255)  # red
        green = int(pow((int((green / clear) * 256) / 255), 2.5) * 255)  # green
        blue = int(pow((int((blue / clear) * 256) / 255), 2.5) * 255)  # blue
